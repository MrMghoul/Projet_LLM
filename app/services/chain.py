...
class SummaryService:
    ...
    async def generate_summary(self, message: str) -> Dict[str, Any]:
        try:
            result = await self.summary_chain.ainvoke({"text": message})
            
            # Vérifiez si les clés attendues sont présentes dans le résultat
            if "full_summary" not in result or "bullet_points" not in result or "one_liner" not in result:
                raise ValueError("Résultat incomplet de la chaîne de résumé")
            
            return {
                "full_summary": result["full_summary"],
                "bullet_points": result["bullet_points"].split("\n"),  # Sépare les points clés par des sauts de ligne
